...
The app now also includes an **In-season FAAB Lab** for guillotine leagues. A shared league-data layer refreshes configured sources every four hours: Yahoo is preferred for authoritative ownership/waiver data, while FantasyPros MyPlaybook can supply the roster, lineup, projections, injuries, opponents, ECR, and schedule context when Yahoo is unavailable. The latest good snapshot and manual CSV remain fallbacks. The Lab calculates recommended/projected-winning/stretch bids, saves weekly reports, and runs automatically at approximately 1:00am Pacific on Tuesdays. Recurring calendar reminders are available in the UI. Optional email delivery requires `RESEND_API_KEY` and `FAAB_REPORT_FROM` Worker secrets.
...
- **League data sources**: configure a FantasyPros MyPlaybook NFL league URL/key from the FAAB Lab. Keys are stored only in KV and are never returned to the browser after saving. `GET /api/league-data` exposes the normalized snapshot that future lineup, waiver, trade, and roster-management modules should consume. FantasyPros currently supplements or replaces roster/projection data; Yahoo remains the preferred source for the complete newly dropped/available pool because the MyPlaybook matchup feed does not expose that full list.
