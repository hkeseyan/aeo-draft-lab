...
- ✅ 2026-09-08 — **ADP/ECR refresh for AEO-Keepers + AEOK Auction, draft night.** ECR pulled fresh from FantasyPros' half-PPR cheatsheet page (`ecrData` embed, 978 players, `last_updated` = today) — 250/250 match for AEO-Keepers, 334/338 for AEOK Auction (4 deep names — Jayden Higgins, Ricky Pearsall, Calvin Austin III, Nick Chubb — aren't in FP's current half-PPR top-978 at all; left their prior ecr/tier untouched rather than guess). Tried the documented ADP method (FantasyPros' ADP page needs JS) via headless Chromium (Playwright, pre-installed in this environment) — every navigation reset mid-TLS-handshake, not just on FantasyPros (same failure on example.com), confirmed via the agent-proxy status endpoint as a proxy-side limitation (`ws_closed_mid_exchange`), not a site block. **New finding for next time:** this environment cannot browser-render JS-heavy pages through the egress proxy at all — don't re-attempt Playwright for ADP, go straight to an alternative source. Substituted FantasyFootballCalculator's public half-PPR 12-team ADP API (`fantasyfootballcalculator.com/api/v1/adp/half-ppr?teams=12&year=2026`, no auth, real composite from 1837 live 2026 drafts through today) for ~185-190 players per league; deeper bench names kept their prior ADP. Updated both leagues' live KV `playersCsv` directly (already deployed to production, zero deploy risk) plus `players-2026.csv` and the `PLAYERS_CSV_AEO` fallback embed so the repo matches. `fp_adp`/`yahoo_adp` columns in `players-2026.csv` are legacy/unused by the app (confirmed via `parsePlayers()`) and were left stale — not worth fabricating without real sources.

- ✅ 2026-09-12 — **Second ADP/ECR refresh, same method as 9/8.** FantasyPros half-PPR ecrData (985 players, `last_updated` 9/10) + FantasyFootballCalculator half-PPR 12-team ADP (377 live drafts through today) for AEO-Keepers and AEOK Auction. Same 4 names still missing from FP's half-PPR top-985 (Higgins, Pearsall, Austin III, Chubb) — left untouched. Both leagues' live KV playersCsv, `players-2026.csv`, and the `PLAYERS_CSV_AEO` fallback embed updated and deployed.
